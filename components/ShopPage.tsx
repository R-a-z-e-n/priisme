...
const ProductCard: React.FC<{product: Product, onTryOn: (imageUrl: string) => void}> = ({ product, onTryOn }) => (
    <div className="group relative">
        ...
        <button 
            onClick={() => onTryOn(product.imageUrl)} 
            className="mt-2 w-full bg-sky-500 text-white text-xs py-1.5 rounded-md font-semibold hover:bg-sky-600 transition-colors"
        >
            AR Try-On
        </button>
    </div>
...
const ShopPage: React.FC<ShopPageProps> = ({ pageType, onSelectProductForTryOn }) => {
    ...
    return (
        <div className="p-4 md:p-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-10">
                {filteredProducts.map(product => <ProductCard key={product.id} product={product} onTryOn={onSelectProductForTryOn} />)}
            </div>
        </div>
    );
};
